...
def build_superpotential(grid_size=360):
    """Build W(φ,ψ) = -ln(p + ε) from a von Mises mixture model
    of the Ramachandran distribution."""
    ...
    # Von Mises mixture components (empirical Ramachandran)
    components = [
        # (weight, mu_phi, mu_psi, kappa_phi, kappa_psi)
        (0.40, -1.10, -0.75, 20.0, 20.0),   # alpha-helix
        (0.25, -2.09,  2.27, 15.0, 12.0),    # beta-sheet
        (0.10, -1.31,  2.62, 10.0,  8.0),    # polyproline II
        (0.08,  1.00,  0.82, 15.0, 15.0),    # left-handed helix
        (0.05, -1.40,  0.50,  5.0,  5.0),    # bridge region
        (0.04, -2.50, -0.70, 10.0, 10.0),    # extended near beta
        (0.03, -1.05,  2.90,  8.0,  6.0),    # ppII variant
        (0.03, -1.55,  2.80, 10.0,  8.0),    # another ppII mode
        (0.01,  0.00,  3.14,  3.0,  3.0),    # rare left-handed ext
        (0.01, -2.80,  0.00,  3.0,  3.0),    # rare extended
    ]

    density = np.zeros_like(PHI)
    for w, mu_p, mu_s, kp, ks in components:
        vm_phi = np.exp(kp * np.cos(PHI - mu_p))
        vm_psi = np.exp(ks * np.cos(PSI - mu_s))
        density += w * vm_phi * vm_psi
...
def lookup_W(W_grid, phi_grid, psi_grid, phi, psi):
    """Bilinear lookup of W at (phi, psi) in radians."""
    ...
    pi = (psi_w - psi_grid[0]) / dpsi

    fi = fi % len(phi_grid)
    pi = pi % len(psi_grid)

    i0 = int(fi) % len(phi_grid)
    j0 = int(pi) % len(psi_grid)
    ...
    val = (W_grid[i0, j0] * (1 - df) * (1 - dp) +
           W_grid[i1, j0] * df * (1 - dp) +
           W_grid[i0, j1] * (1 - df) * dp +
           W_grid[i1, j1] * df * dp)
    return val
